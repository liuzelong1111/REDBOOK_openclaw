...
description: |
  ...
  适用场景：发布图文、发布视频、AI生图、仅启动测试浏览器、获取登录二维码、首页推荐抓取、搜索笔记、评论互动、抓取内容数据。
metadata:
  trigger: 发布内容到小红书
...
- **配图方式优先级**：用户指定配图方式时按用户要求；未指定时：
  - 用户要求"AI 生图"或"文生图" → 调用 AI 生成
  - 用户描述图片内容 → 调用 AI 生成
  - 用户提供 URL/本地路径 → 使用原有方式
...
用户直接描述想要什么样的图：
```
帮我发一篇关于"猫咪"的小红书，配一张可爱猫咪的图片
```

### 方式二：根据文案内容自动匹配生成
用户只给主题，AI 根据文案关键词生成匹配的图片：
```
帮我写一篇关于"居家收纳技巧"的小红书，用AI生成配图
```

### 方式三：原有方式（URL/本地图片）
用户直接提供图片 URL 或本地路径：
```
帮我发一篇小红书，配图用这张图 https://example.com/xxx.jpg
```
...
```bash
# 生图并保存到本地
python scripts/image_gen.py -p "图片描述" -o "images/output.jpg"

# 示例：根据文案生成匹配的图
python scripts/image_gen.py -p "a cozy living room with organized storage boxes" -o "images/cover.jpg"
```

**支持的生图 API**：
- 硅基流动 FLUX.1-schnell（默认，已配置）
- 可在 `config/api_config.json` 中切换其他模型
